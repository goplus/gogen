...
func TestGenericTypeOverloadMethod(t *testing.T) {
	...
	v := pkg.NewParam(token.NoPos, "v", tyData, false)
	pkg.NewFunc(nil, "bar", types.NewTuple(v), nil, false).BodyStart(pkg).
		...
		Debug(func(cb *gogen.CodeBuilder) {
			cb.Member("size", 0, gogen.MemberFlagMethodAlias)
		}).
		Call(0).EndInit(1).EndStmt().
		Val(v).
		Debug(func(cb *gogen.CodeBuilder) {
			cb.Member("add", 0, gogen.MemberFlagMethodAlias)
		}).
		...
		Debug(func(cb *gogen.CodeBuilder) {
			cb.Member("indexOf", 0, gogen.MemberFlagMethodAlias)
		}).
